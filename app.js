...
client.on("message", async message => {
  ...
  if(command === "help") {
    ...
    .addField("myinfo", "Tells you info about yourself is currently Disabled")
  ...
  if(command === "kick") {
    ...
    if(!message.member.roles.some(r=>["-=[Founder]=-", "-=[Sub-Founder]=-", "-=[Administrator]=-", "-=[Moderator]=-", "Trainee"].includes(r.name)) )
    ...
    if(!member)
      return message.reply("Please mention a valid member of this server! Example: /kick @Andrew Being Annoying");
    if(!member.kickable)
      return message.reply("I cannot kick this user! Do they have a higher role? Do I have kick permissions? Are you doing the command wrong? Example: /kick (user) (reason)");
  ...
  if(command === "myinfo") {
    ...
    let myinfoembed = new Discord.RichEmbed()
    .setDescription("Your Information")
    .setColor("#15f153")
    .addField("Your Name", message.author)
    .addField("Your ID", user.id);
  ...
  if(command === "ban") {
    ...
    if(!message.member.roles.some(r=>["-=[Founder]=-", "-=[Sub-Founder]=-", "-=[Administrator]=-", "-=[Moderator]=-"].includes(r.name)) )
      return message.reply("Sorry, you don't have permissions to use this!");
